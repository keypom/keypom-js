...
async function performActions(params) {
    const { near, config, trialAccountId, trialAccountSecretKey, contractAccountId, actionsToPerform, } = params;
    // Set the trial key in the keyStore
    const keyStore = near.connection.signer.keyStore;
    await keyStore.setKey(config.networkId, contractAccountId, crypto_1.KeyPair.fromString(trialAccountSecretKey));
