...
export const tryGetSignInData = async ({
    ...
    if (connectionSplit.length > 1) {
        ...
        try {
            ...
            decodedString = Buffer.from(connectionString, "base64").toString(
                "utf-8"
            );
            ...
            if (!connectionParam) {
                console.error(
                    "Error decoding connection data, ensure connection parameter exists and it is base64 or URL encoded"
                );
                return null;
            }
            console.log("connection param from URL: ", connectionParam);
            decodedString = Buffer.from(connectionParam, "base64").toString(
                "utf-8"
            );
            connectionData = JSON.parse(decodedString);
        }
