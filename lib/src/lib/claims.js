...
*/
export const claim = ({ secretKey, accountId, newAccountId, newPublicKey, password, fcArgs }) => __awaiter(void 0, void 0, void 0, function* () {
    const { networkId, keyStore, attachedGas, contractId, contractAccount, receiverId, execute, fundingAccountDetails, near, } = getEnv();
    ...
    const dropInfo = yield getDropInformation({ secretKey });
    const keyInfo = yield getKeyInformation({ secretKey });
    if (fcArgs) {
        assert(dropInfo.fc, 'Cannot pass in fcArgs for non-FC drops.');
        const curMethodData = yield getCurMethodData({ secretKey });
